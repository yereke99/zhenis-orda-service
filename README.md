...
- `FREE_LESSON_UPLOAD_DIR` default `UPLOAD_DIR/free-lessons`, used for uploaded free lesson images
- `WHATSAPP_SALES_PHONE` digits or formatted phone for book sales WhatsApp links
- `MAX_BOOK_IMAGE_BYTES` default `5242880`
- `MAX_FREE_LESSON_IMAGE_BYTES` default `5242880`
...
- Docker Compose stores uploaded images under the persistent `app-uploads` volume. Book images are saved in `/app/uploads/books` and free lesson images are saved in `/app/uploads/free-lessons` by default.
